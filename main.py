...
def main():
    try:
        ...
        for _ in range(config.MAX_SCROLLS):
            try:
                ...
                logger.error(f"Error scrolling or extracting jobs: {e}")

        driver.quit()
    except Exception as e:
        logger.error(f"Error setting up or quitting webdriver: {e}")
    ...
    try:
        ...
        logger.error(f"Error saving job data to Excel: {e}")
